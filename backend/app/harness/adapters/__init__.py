...
- messages.py: canonical 消息模型 + 消息判断函数
- providers.py: LLM_PROVIDER 映射表 + Protocol enum + resolve_provider
...
from app.harness.adapters.providers import PROVIDER_PROTOCOLS, Protocol, resolve_provider
...
def get_adapter(protocol: Protocol) -> LLMAdapter:
    """按协议拿 adapter 实例（注册表查询，不感知具体格式）"""
    try:
        return _ADAPTERS[protocol]
    except KeyError:
        raise ValueError(f"未知协议: {protocol}（可选: {', '.join(p.value for p in _ADAPTERS)}）") from None
