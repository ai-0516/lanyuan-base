...
2. 后端读取 header x-wx-openid（平台注入，外部无法伪造）→ 查/建 User（openid 唯一）
...
| 方法 | 路径 | 说明 | 请求体 | 响应 |
|------|------|------|--------|------|
| POST | `/auth/login` | 微信登录 | `{ code }`（云托管链路可省略——平台注入 `x-wx-openid` header 优先） | `{ token, user }` |
